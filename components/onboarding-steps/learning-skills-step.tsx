...
interface LearningSkillsStepProps {
  formData: {
...
export function LearningSkillsStep({ formData, updateFormData, onNext, onBack }: LearningSkillsStepProps) {
  ...
  const handleSelect = (skill: string) => {
    if (formData.learningSkills.includes(skill)) {
      ...
      updateFormData("learningSkills", [...formData.learningSkills, skill])
    }
    setError("")
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Select skills you're interested in learning from others in the community.
          </p>
        </div>
                ...
                >
                  <span className="truncate">
                    {formData.learningSkills.length > 0
                      ? `${formData.learningSkills.length} skill${formData.learningSkills.length > 1 ? "s" : ""} selected`
                      : "Select skills..."}
                  </span>
                          ...
                          .map((skill) => (
                            <CommandItem key={skill} value={skill} onSelect={() => handleSelect(skill)}>
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4",
                                  formData.learningSkills.includes(skill) ? "opacity-100" : "opacity-0",
                                )}
                              />
                              {skill}
                            </CommandItem>
